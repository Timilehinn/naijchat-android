...
function SearchSreen({navigation}) {
    ...
    useEffect(()=>{
        async function getAllTopics() {
            const res = await axios.get('http://192.168.43.223:3333/search-topics')
    ...
    // const funcFindTopic= async (e)=>{
        ...
        const found = allTopics.filter(topic => {
            return topic.title.toLowerCase().indexOf(searchValue.toLowerCase()) !== -1
        })
        // setFoundValue(found)
    ...
    return (
        <View style={{flex:1,backgroundColor:darkMode.backgroundColor}}>
            ...
            <ScrollView>
            {found.map(val=>(
                <TouchableOpacity key={val.id} onPress={()=>navigation.navigate('chat_screen',{topic:val})}>
                    <View style={{display:'flex',alignItem:'center',padding:18,backgroundColor:darkMode.backgroundColor,borderBottomWidth:.5,borderColor:darkMode.border}}>
                        <Text style={{color:'grey',textAlign:'center'}}>Original Poster @{val.creator} {val.is_poster_verified == 'true' ? <IonIcon name="ios-checkmark-circle-outline" size={15} color='#5cab7d'/> : <></>}</Text>
                        <Text style={{textAlign:'center',fontSize:17,color:darkMode.color,}}>
